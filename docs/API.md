...
`GET /api/v1/collection` — Query: `game` (Spielcode), `set` (Set-`id`, spielübergreifend eindeutig), `search`, `page`, `limit`, `sort` (`newest`|`oldest`|`name`|`quantity`|`value`), `condition`, `language`, `first_edition` (`1`).
