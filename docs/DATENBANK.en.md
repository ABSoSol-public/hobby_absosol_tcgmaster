...
## Yugipedia as a secondary source for Yu-Gi-Oh! gaps (`card_sets.de_prefix`, since 2026-07-31)

Two gaps not covered by YGOPRODeck (primary source), found via user feedback "I often see English names/numbers when German is selected":

- **Missing German translation**: YGOPRODeck has no German release for roughly 19% of the catalog (`name_de`/`card_text_de` stay `null`) — spot-verified live against the source to be genuine data gaps, not an import bug. Yugipedia (yugipedia.com) still carries a German translation for some of these cards (likely sourced from the Master Duel localization, which sometimes exists ahead of the paper release) — taken as a supplement, **never overwrites** a translation already delivered by YGOPRODeck.
- **Collector number prefix**: YGOPRODeck fundamentally provides no German collector numbers (`card_prints.collector_number` is always the English one, e.g. `LOB-EN001`). The previous frontend hint (`frontend/src/cardNumbers.ts` → `deVariantHint()`) simply textually replaced the language code with "DE" — demonstrably wrong for early sets: "Legend of Blue Eyes White Dragon" actually uses the prefix `LOB-G`, not `LOB-DE` (verified against Yugipedia's set infobox). New column `card_sets.de_prefix`, populated best-effort; if no prefix is known, the old guessing logic remains as a fallback (modern sets do consistently use "-DE"). The Yugipedia lookup deliberately only runs for sets **before 2005** (or with an unknown release date) — live evaluation shows "-G" instead of "-DE" only ever occurs in that early window (last hit: Ancient Sanctuary, 2004-05-31; consistently "-DE" afterwards, including newer spot checks like the 2011/2013 Collectible Tins sets). Saves roughly 90% of the requests (67 instead of 636 sets checked).

**Technical**: `backend/src/services/importers/yugipedia.ts`. Yugipedia offers no structured query API (`action=cargoquery` doesn't exist) — the raw wikitext page is fetched instead (`action=query&prop=revisions`, page title = English card/set name) and the relevant infobox fields (`de_name`, `de_text`, `de_prefix`) are extracted via regex. Safety net against name collisions: the card infobox's `password` field (Yugipedia's name for the numeric card ID) must match the expected YGOPRODeck ID, otherwise the translation is discarded.

Runs **only on `--force`** imports (not the daily delta-import cron) — otherwise every automatic run would re-check all cards/sets without a known German translation or prefix against Yugipedia, even though that rarely changes (best-effort, no "already checked, found nothing" tracking). To pull in new translations/prefixes, run the Yu-Gi-Oh! import manually with `--force` (`npm run import:game -- yugioh --force`).
